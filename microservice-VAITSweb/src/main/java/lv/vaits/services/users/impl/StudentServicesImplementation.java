...
public class StudentServicesImplementation implements IStudentServices {
	...
	@Override
	public Student createNewStudent(String name, String surname, String personcode, Long id_user, String matriculaNo,
			boolean financialDebt) {
		return studentRepo.save(new Student(name, surname, personcode, matriculaNo, financialDebt, id_user));
	}
	...
	public Student updateStudentById(Long id, String name, String surname, String personcode, Long id_user,
		...
		if (studentRepo.existsById(id)) {
			...
			updatedStudent.setId_user(id_user);
	...
	public Workbook exportStudentsToExcel() {
		...
		for (Student student : students) {
			...
			// dataRow.getCell(3).setCellValue(student.getUser().getEmail());
	...
	public void importStudentsFromExcel(InputStream excelFile) throws IOException {
		...
		while (rowIterator.hasNext()) {
			...
			String matriculaNo = row.getCell(3).getStringCellValue();
			boolean financialDebt = row.getCell(4).getBooleanCellValue();

			if (!studentRepo.existsByMatriculaNo(matriculaNo)) {
				// User currentUser = users.get(currentUserIndex);
				createNewStudent(name, surname, personcode, currentUserIndex, matriculaNo, financialDebt);
				currentUserIndex++;
	...
	public XWPFDocument exportStudentsToWord() {
		...
		for (Student student : students) {
			...
			dataRow.getCell(3).setText(student.getMatriculaNo());
			dataRow.getCell(4).setText(String.valueOf(student.isFinancialDebt()));
		}
